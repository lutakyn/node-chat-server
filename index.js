...
//DB Connection
mongoose.connect(
  'mongodb+srv://lutakyn:lutakyn@1234@cluster0.q4yo4ax.mongodb.net/chat?retryWrites=true&w=majority',
...
app.post("/login", (req, res) => {
  console.log('in login sessions')
  const query = User.find({ id: req.body.id });
  query.exec().then(data => {
    console.log('in login sessions',data)
...
app.get("/", (req, res) => {
    ...
    res.json({message: "Connected to me"});
});
...
io.on("connection", socket => {
  ...
  socket.on("storeClientInfo", function(data) {
    console.log(data.customId + " is Connected");
